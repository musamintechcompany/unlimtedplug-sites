<x-admin-layout>
    <div class="max-w-4xl mx-auto">
        <h1 class="text-3xl font-bold text-gray-900 dark:text-white mb-6">Edit Role</h1>

        <form method="POST" action="{{ route('admin.roles.update', $role) }}" class="bg-white dark:bg-gray-800 rounded-lg shadow p-6 space-y-6">
            @csrf
            @method('PATCH')
            ...
            <div>
                ...
                <input type="text" name="name" value="{{ old('name', $role->name) }}" required class="w-full px-4 py-2 border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-700 text-gray-900 dark:text-white focus:ring-2 focus:ring-blue-500 focus:border-transparent">
                    ...
                    <div class="border border-gray-200 dark:border-gray-700 rounded-lg p-4">
                        ...
                        <div class="grid grid-cols-2 gap-3">
                            ...
                            <label class="flex items-center gap-2 cursor-pointer">
                                <input type="checkbox" name="permissions[]" value="{{ $permission->id }}" {{ $role->permissions->contains($permission->id) ? 'checked' : '' }} class="w-4 h-4 rounded border-gray-300 dark:border-gray-600">
